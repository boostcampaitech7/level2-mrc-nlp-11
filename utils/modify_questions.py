...
def modify_questions(questions, mode):
    ...
    response_format = rf"""
        [
            {{"original": "올레 폰 보이스트 이후에 시장으로 임명된 사람은 누구인가?", "paraphrased": "올레 폰 보이스트 다음에 시장으로 선임된 인물은 누구인가?"}},
    ...
    output_path = (
        ...
        + f"/data/paraphrased_{mode}_questions.json"
    )
...
def main():
    ...
    modify_questions(train_questions, "train")
    modify_questions(validation_questions, "validation")
